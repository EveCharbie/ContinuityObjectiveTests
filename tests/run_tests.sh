...
for ((idx_random=0; idx_random<100; idx_random=idx_random+1)); do
    ...
    for weight in ${weights[*]}; do
        for weight_sphere in ${weight_spheres[*]}; do
            for max_iter in ${max_iters[*]}; do
                python3 obstacle_work_around.py unconstrained $idx_random --iters1 ${max_iter}  --iters2 10000 --weight ${weight} --weight_sphere ${weight_sphere}
            done
        done
    done
    ...
    for weight in ${weights[*]}; do
        for max_iter in ${max_iters[*]}; do
            python3 obstacle_work_around.py objective_sphere $idx_random --iters1 ${max_iter} --iters2 10000 --weight ${weight}
        done
    done
    ...
    for weight_sphere in ${weight_spheres[*]}; do
        for max_iter in ${max_iters[*]}; do
            python3 obstacle_work_around.py objective_continuity $idx_random --iters1 ${max_iter} --iters2 10000 --weight_sphere ${weight_sphere}
        done
    done
