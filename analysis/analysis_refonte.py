...
def compute_max_transpersion(m, x, u, t, nb_shooting):
    ...
    for i in range(nb_sub_intervals * nb_shooting + 1):
        transpersion[i] = transpersion_dist_max(x_sub_interval[:, i])
...
def graph_convergence(properties_constrained_converged, 
    ...
    ax[1].set_title("Objective sphere", fontsize=12)
    ax[2].set_title("Objective continuity", fontsize=12)
...
def graph_kinmatics_each_good(properties_constrained_converged,
    ...
    def plot_lines(key_word, linestyle, properti, state, nb_shooting, cmap, ax):
        ...
        for i in range(np.shape(properti)[0]):
            ...
            if i == 0:
                plt_0 = ax[0].plot(time_vector, state[0, :, i],
    ...
    plt.plot(0, 0, '--', color=cmap(0.25), label="Solutions in clusters #0, #1, #2")
    plt.plot(0, 0, '-', color=cmap(0.25), label="Solution in cluster #3")
    plt.plot(0, 0, ':', color=cmap(0.25), label="Solutions without transpersion")
    plt.legend(loc='upper center', bbox_to_anchor=(-23, -0.15), ncol=3, frameon=True)
...
def plot_stats(constrained_convergence_rate,
    ...
    colors = [cmap(0), cmap(0.25), cmap(0.75), cmap(0.99)]
    ...
    for i in range(4):
        ax[1].bar(1, pourcentage_cost_constrainted[i], bottom=sum_constrained, width=0.4, color=colors[i], label="Cost {:} ".format(round(mean_cluster[i], 2)) + u'\u00B1' + " {:.1e}".format(std_cluster[i]))
...
def plot_marker_trajectory(states_objective_sphere_final_converged, idx_clusters_sphere_final):
    ...
    for i in range(4):
        ax.plot(markers[i, :, 1], markers[i, :, 2], color=colors[i], label=f'Cluster #{i}')
...
plot_marker_trajectory(states_objective_sphere_final_converged, idx_clusters_sphere_final)
...
graph_kinmatics_each_good(properties_constrained_converged,
                properties_objective_sphere_final_converged,
                properties_objective_continuity_final_converged,
                states_constrained_converged,
                states_objective_sphere_final_converged,
